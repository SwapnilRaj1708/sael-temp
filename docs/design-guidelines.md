...
| `--about-shape-aspect` | `973 / 620` | …the chamfered shape inside it — `homepage/about/mask.svg`'s viewBox, which its path fills exactly. **Was** `354.45 / 225.97`, `14.svg`'s path bounding box, until 2026-09-02; the same ratio to four decimals |
| `--aspect-map-india` | `311.33 / 337.45` | Dotted India map viewBox |

### The About SAEL artwork — assembled, 2026-09-01

Three supplied layers, put together in the browser: `homepage/about/burning-crop.png`
masked into `homepage/about/mask.svg`'s silhouette, graded by `--gradient-shape`, with
`homepage/about/sardar-kid-cropped.png` standing in front of it.
...
reason it is expressed this way. `homepage/about/sardar-kid-cutout.png`, the composite
...
| `--mask-about-shape` | `url("data:image/svg+xml,…")` | `homepage/about/mask.svg`'s path verbatim, with `fill` changed to opaque white — a mask reads alpha, and the ramp is `--gradient-shape`'s job. Inlined rather than `url(../assets/…)` — a `url()` inside a custom property is an unparsed token stream, and a mask that fails to resolve fails silently |
