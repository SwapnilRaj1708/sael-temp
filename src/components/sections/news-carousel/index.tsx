  ...
  return (
    ...
    >
      <div className="flex w-full flex-col gap-flow">
        <Container>
          {/* Regular weight and centred, as the design sets it. */}
        ...
        >
          {items.map((item) => (
            <li key={item.id} className="w-news-card shrink-0 snap-start">
              <article
                className={cn(
                  'border-border bg-surface group relative flex h-full flex-col border',
                  ...
                  'hover:border-brand-red/40 hover:-translate-y-1 hover:shadow-card-hover',
                  'focus-within:-translate-y-1 focus-within:shadow-card-hover',
                  'motion-reduce:transition-none motion-reduce:hover:translate-y-0',
                )}
              >
                <div className="bg-surface-deep relative aspect-(--aspect-news-thumb) w-full overflow-hidden">
                  {item.imageUrl !== null && (
                    <Image
                ...
                <div className="flex flex-1 flex-col items-start gap-stack p-4">
                  ...
                  {/* `line-clamp-3` needs no `overflow-hidden` of its own — it
                      ...
                      link below. */}
                  <h3 className="text-body-sm line-clamp-3 font-bold transition-colors duration-(--duration-micro) group-hover:text-brand-red">{item.title}</h3>

                  <Button
                    href={item.href}
                    variant="ghost"
                    size="sm"
                    className="mt-auto px-0 after:absolute after:inset-0 after:content-['']"
                  >
