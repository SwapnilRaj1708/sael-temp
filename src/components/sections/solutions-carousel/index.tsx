...
export function SolutionsCarousel({
  ...
  return (
    ...
    >
      <div className="flex w-full flex-col gap-flow [--rail-card-w:var(--solution-card-w)]">
        ...
        <Reveal order={4}>
          ...
          >
            {slides.map((slide) => (
              <li
                key={slide.id}
                // The centred card is the subject; its neighbours are context.
                // `data-rail-active` is written by <CardRail> as the rail
                // scrolls, so the dimming follows the snap rather than a hover.
                className={cn(
                  'rail-card shrink-0 snap-center',
                  'transition-opacity duration-(--duration-card)',
                  'data-[rail-active=false]:opacity-40',
